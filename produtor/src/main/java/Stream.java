        ...
        Listener = new StatusListener() {
            public void onStatus(Status status) {
                ...
                if (status.getGeoLocation() == null){
                    geo = new GeoLocation(0,0);
                }else  {
                    geo = status.getGeoLocation();
                }
                tt = new Tweet(status.getUser().getName().replace("'",""),
                            status.getText().replace("'",""), status.getCreatedAt(),  status.getId(),
                            status.getSource().replace("'",""),
                            status.isTruncated(), geo,
                            status.isFavorited(), status.getContributors().toString());
